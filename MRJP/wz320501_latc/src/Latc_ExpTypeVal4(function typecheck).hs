...
multipleTypesMatch (PIdent ((x,y),name)) (x1,y1) (exp:exps) (t:ts) i = do
	...
	if t2==t
		then multipleTypesMatch (PIdent ((x,y),name)) (x1,y1) exps ts (i+1)
...
checkExpTypeVal (EMul exp1 (Div (PDiv ((x,y),_))) exp2) = do
	...
	if ((type1==Int) && (type2==Int))
		then case val2 of
			Nothing -> return (Int,Nothing)
			Just (Right 0) -> error ("divide by 0 at line "++show(x)++", column "++show(y))
...
checkExpTypeVal (EMul exp1 (Mod (PMod ((x,y),_))) exp2) = do
	...
	if ((type1==Int) && (type2==Int))
		then case val2 of
			Nothing -> return (Int,Nothing)
			Just (Right 0) -> error ("modulo by 0 at line "++show(x)++", column "++show(y))
			Just (Right n2) -> case val1 of 
				Nothing -> return (Int,Nothing)
		...
		then case type1 of
			...
			Str -> case val1 of
				...
				Just (Left (Left s1)) -> case val2 of 
					Nothing -> return (Str,Nothing)
					Just (Left (Left s2)) -> return (Str,Just (Left (Left (s1++s2))))
