...
duplicateAndAssVar :: String -> Val -> Int-> StEnv Env	--lokacja
duplicateAndAssVar name val newlevel= do
	env <- ask
	st <- getSt
	case Map.lookup name env of
...
checkStmt :: Stmt ->Type -> Int -> Int -> StEnv Env			--wyrażenie -> typ funkcji -> poziom -> poziom bezpieczny

checkStmt Empty _ _ _ = ask

checkStmt (BStmt (Block b)) ft level slevel = do
	checkBlock b ft (level+1) slevel
	ask
	
checkStmt (Decl t l) _ level _ = checkDecl t l level		--TODO czy slevel

checkStmt (Ass (PIdent ((x,y),name)) exp) _ _ slevel= do	--jeśli przypisujemy na zmienną z level<slevel, to źle TODO przetestować
	env <- ask
	st <- getSt
	(type2,val) <- checkExpTypeVal exp
	case (Map.lookup name env) of
		...
		Just ((x1,y1),loc,level2) -> case (Map.lookup loc st) of
			Just (t,_) -> if t==type2
				then if level2<slevel 
					then do
						duplicateAndAssVar name val slevel
					else do
						let s = Map.insert loc (t,val) st
						putSt s
						ask
...
checkStmt (Incr (PIdent ((x,y),name))) _ _ slevel= do
	env <- ask
	st <-getSt
	case (Map.lookup name env) of
		Nothing -> error ("error in line "++show(x)++", column "++show(y)++" variable "++name++" not declared")
		Just ((x1,y1),loc,level2) -> case (Map.lookup loc st) of
			...
			Just (Int,Just (Right n))-> if level2<slevel 
				then do
					duplicateAndAssVar name (Just (Right (n+1))) slevel
				else do
...
checkStmt (Decr (PIdent ((x,y),name))) _ _ slevel= do
	env <- ask
	st <-getSt
	case (Map.lookup name env) of
		Nothing -> error ("error in line "++show(x)++", column "++show(y)++" variable "++name++" not declared")
...
checkStmt (Ret (PReturn ((x,y),_)) exp) ft _ _ = case ft of
...
checkStmt (VRet (PReturn ((x,y),_))) ft _ _ = case ft of
	Void -> ask
	_ -> error ("error in line "++show(x)++", column "++show(y)++"return without argument in non-void function")

checkStmt (Cond (PIf ((x,y),_)) exp stm) ft level slevel = do			--TODO tutaj wejście do bloku warunkowego lub False dać level+2 i slevel=level+1
	(type1,val) <- checkExpTypeVal exp
	case type1 of
		Bool -> case stm of
			Decl _ _ -> error ("error in line "++show(x)++", column "++show(y)++" bare declaration in if")
			_ -> case val of
				Just (Left (Right True)) -> checkStmt stm ft level slevel	--TODOTODO to się wykona zawsze - można usunąć if warunek itp. jak blok to ok, jak deklaracja to się wywal
				Just (Left (Right False)) -> checkStmt stm ft (level+1) (level+1)	--TODOTODO to się i tak nie wykona, ale warto sprawdzić otypowanie - można by zrobić pierwszą optymalizację 
				Nothing -> checkStmt stm ft (level+1) (level+1)						--TODOTODO to się może wykonać - jak deklaracja to się wywal, jak przypisanie, to daj wartość na nieokreśloną, jak blok to dalej ... z nieokreślonością, jak if czy coś to ???, czyli ustawić slevel jako level+1,,,
		_ -> error ("error in line "++show(x)++", column "++show(y)++"if condition of non-boolean type")
		
checkStmt (CondElse (PIf ((x,y),_)) exp stm stm2) ft level slevel = do	
	(type1,val) <- checkExpTypeVal exp
	case type1 of
		Bool -> case stm of
			Decl _ _ -> error ("error in line "++show(x)++", column "++show(y)++" bare declaration in if")
			_ -> case stm2 of
				...
				_ -> case val of
					Just (Left (Right True)) -> do
						env2 <- checkStmt stm ft level slevel
						checkStmt stm2 ft (level+1) (level+1)			--TODO przecież tutaj zmieniam stan zewnętrzny, a nie powinienem
					...
					Just (Left (Right False)) -> do
						checkStmt stm ft (level+1) (level+1)			--TODO przecież tutaj zmieniam stan zewnętrzny, a nie powinienem
						env2 <- checkStmt stm2 ft level slevel
						return env2
					Nothing -> do
						checkStmt stm ft (level+1) (level+1)
						checkStmt stm2 ft (level+1) (level+1)
						ask
		_ -> error ("error in line "++show(x)++", column "++show(y)++"if condition of non-boolean type")
...
--checkStmt (While (PWhile ((x,y),_)) exp stm) ft level slevel = do
...
checkStmt (SExp exp) _ _ _ = do
	checkExpTypeVal exp
	ask


checkStmt _ _ _ _ = ask --TODO usunąć



checkBlock :: [Stmt] -> Type -> Int -> Int -> StEnv ()
checkBlock (stm:stmts) ft level slevel= do
	env <- checkStmt stm ft level slevel
	(local (\x -> env) (checkBlock stmts ft level slevel))
	return ()
	
checkBlock  [] _ _ _= return ()
...
checkFunction (FnDef t (PIdent ((x,y),name)) args (Block b)) = do
	env <- checkArgs args
	(local (\x -> env) (checkBlock b t 2 0))
