...
defaultVal (Array _) = Nothing

defaultValExpr :: Type -> Expr
defaultValExpr Int = ELitInt 0
defaultValExpr Bool = ELitFalse
defaultValExpr Str = EString ""
defaultValExpr (Array _) = (ENewArr (PNew ((0,0),"new")) Int (ELitInt 0))		--TODO to trzeba zmienić, a najlepiej dać coś takiego, że wykryje, że tablica nie zainicjalizowana, lub dać inicjalną wartość jak w treści itp.
